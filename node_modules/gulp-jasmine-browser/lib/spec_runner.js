...
var SpecRunner = function (_File) {
  ...
  function SpecRunner() {
    ...
    var useSourcemappedStacktrace = !options.console && options.sourcemappedStacktrace;
    privates.set(_this, { files: new _set2.default() });
    [].concat(resolveJasmineFiles('path', 'cssFiles'), resolveJasmineFiles('path', 'jsFiles'), options.console ? ['console.js', 'console_boot.js'] : resolveJasmineFiles('bootDir', 'bootFiles'), [useSourcemappedStacktrace && require.resolve('sourcemapped-stacktrace/dist/sourcemapped-stacktrace.js'), useSourcemappedStacktrace && 'sourcemapped_stacktrace_reporter.js']).filter(Boolean).forEach(function (fileName) {
